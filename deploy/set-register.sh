...
#  Kod ham, parol ham ekranda ko'rinmaydi va buyruqlar tarixiga tushmaydi.
# =====================================================================
set -euo pipefail
...
case "$CMD" in
  list)
    ssh "$HOST" "python3 - <<'PY'
import json, pathlib
f = pathlib.Path('$APP/data/users.json')
d = json.loads(f.read_text()) if f.exists() else {}
print('Ro\\'yxatdan o\\'tganlar:', len(d))
for r in d.values(): print(' ', r['name'], '→', r['uid'], r.get('createdAt','')[:10])
PY"
    ;;
  pass)
...
PY"
  ...
  door)
    ...
    printf '%s\n%s\n' "$REG" "$INV" | ssh "$HOST" "python3 - <<'PY'
import pathlib, sys
reg, inv = sys.stdin.readline().strip(), sys.stdin.readline().rstrip('\\n')
f = pathlib.Path('$APP/.env'); out = []
for line in f.read_text().splitlines():
    if line.startswith(('MA_REGISTER=', 'MA_INVITE=')): continue
    out.append(line)
